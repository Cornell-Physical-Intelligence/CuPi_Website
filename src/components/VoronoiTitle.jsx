...
// Tunable parameters (P) now live in ./voronoiConfig so the on-screen control panel can
// mutate them live. The simulation reads P every frame; structural params (sites, sigma,
// warp, …) take effect via the rebuild() hook exposed below.
...
export default function VoronoiTitle({ text = 'CUPI', apiRef }) {
  ...
  useEffect(() => {
    ...
    function rebuild() {
      if (!started || disposed) return;
      buildSim(reduced ? WARM_REDUCED : WARM); // re-seeds rand from P.seed internally
    ...
    // ---- Font arrival ----------------------------------------------------------------
    // Every glyph layer except the border is baked ONCE: measureNatural() fixes the box
    // metrics, buildSim() rasterises maskSim, layout() rasterises maskCanvas and the stem
    // underpasses. drawTitleStroke(), by contrast, re-reads ctx.font on every composite().
    // So if the font resolves differently after start(), the letters stay in the old font
    // while the border switches to the new one — the border lands offset from the glyphs,
    // horizontally most of all, because the per-character advances differ (Playfair 700 is
    // ~12% narrower than the Georgia fallback across "CUPI").
    //
    // That is exactly what a cold cache used to produce: document.fonts.ready is NOT a
    // guard here — with no Playfair request pending it resolves immediately (~50ms in,
    // long before the woff2 lands), because no DOM node renders Playfair 700 and canvas
    // font strings don't kick off a fetch. So: request the face explicitly, wait for it,
    // and if it still shows up late, re-bake every layer instead of leaving them mixed.
    ...
    function rebake() {
      if (!started || disposed) return;
      if (Math.abs(currentAdvance() - uw * 100) < 0.01) return; // same font as we baked with
      measureNatural();
      buildSim(reduced ? WARM_REDUCED : WARM);
      dist = null;
      layout();
    ...
    Promise.resolve(fontLoad)
      ...
      .then(() => {
        ...
        if (started) rebake(); // the fallback beat the font in — re-bake with the real one
        else start();
      });
    // Any other late font activity (a swap, a second face landing) gets the same treatment;
    // rebake() is a no-op unless the metrics actually moved.
    const onFontsDone = () => rebake();
    ...
    const onPointerMove = (e) => {
      ...
      mouse.x = clamp(rx, 0, 1) * simW;
      mouse.y = clamp(ry, 0, 1) * simH;
      mouse.active = true;
      dirty = true;
      ensureLoop();
    };
